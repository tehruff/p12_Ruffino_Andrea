...
/**
 * 
 * Scenario alternativo di Prenotazione Volo in cui si utilizzano input illegali
 * e l'algoritmo ti riporta a riimmettere il valore errato
 * 
 * @author se-17-user
 *
 */
...
public class IllegalInputs {
	...
	public void test() {
		...
		when(test.next()).thenReturn("caltanissetta").thenReturn("parigi").thenReturn("10/8/2017")
						 .thenReturn("milano").thenReturn("frosinone").thenReturn("10/8/2017")
						 .thenReturn("milano").thenReturn("parigi").thenReturn("10/8/2287")
						 .thenReturn("milano").thenReturn("parigi").thenReturn("10/8/2017");
		p.setTest(test);
		...
		when(test.nextInt()).thenThrow(InputMismatchException.class).thenReturn(10).thenReturn(0);	
		p.setTest(test);
		...
		when(test.nextInt()).thenThrow(InputMismatchException.class).thenReturn(100).thenReturn(0).thenReturn(2);	
		p.setTest(test);
		...
		when(test.nextInt()).thenThrow(InputMismatchException.class).thenReturn(100).thenReturn(0);
		p.setTest(test);
		...
		when(test.next()).thenReturn("s").thenReturn("c").thenReturn("scadenza generica").thenReturn("a").thenReturn("y");
		when(test.nextInt()).thenThrow(InputMismatchException.class).thenReturn(30).thenReturn(0).thenReturn(0).thenReturn(0).thenReturn(0).thenReturn(5000);	
		p.setTest(test);
		...
		assertEquals("Sbagliato", 250, p.getImporto(), 0.0);
	}
